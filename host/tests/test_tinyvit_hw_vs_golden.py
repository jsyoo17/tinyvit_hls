# host/tests/test_tinyvit_hw_vs_golden.py

#!/usr/bin/env python3
"""
test_tinyvit_hw_vs_golden.py

Compare TinyVitHWSmall (_hw) activations against pre-dumped golden
...
Usage examples:
  ...
  python host/tests/test_tinyvit_hw_vs_golden.py
  ...
  python host/tests/test_tinyvit_hw_vs_golden.py --num-images 50
"""
...
# ---------------------------------------------------------------------
# Allow "from tinyvit.tiny_vit_hw import create_tiny_vit_5m_hw"
# ---------------------------------------------------------------------
...
# NOTE: you must implement this factory in host/tinyvit/tiny_vit_hw.py
# so that it builds your "HW-style" TinyViT model (e.g., using HWConv2d).
from tinyvit.tiny_vit_hw import create_tiny_vit_5m_hw  # type: ignore
...
def compare_tensors(name: str, y_gold: torch.Tensor, y_hw: torch.Tensor) -> Dict[str, Any]:
    if y_gold.shape != y_hw.shape:
        raise RuntimeError(
            f"Shape mismatch for {name}: golden {tuple(y_gold.shape)} vs hw {tuple(y_hw.shape)}"
        )

    diff = (y_gold - y_hw).abs()
    ...
    print(f"  hw     shape: {tuple(y_hw.shape)}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare TinyVitHWSmall activations vs golden dumps."
    )
    parser.add_argument(
        "--num-images",
        type=int,
    ...
    # ---------------------------------------------------------
    # Create HW model & load weights
    # ---------------------------------------------------------
    models_dir = PROJ_ROOT / args.models_dir
    models_dir.mkdir(parents=True, exist_ok=True)

    # HF cache env vars, same as SW tester
    ...
    hw_model = create_tiny_vit_5m_hw(
        models_dir=models_dir,
        state_dict_fname=args.state_dict_fname,
        device=device,
    )
    hw_model.eval()
    print("[INFO] TinyVitHWSmall created and set to eval().\n")

    # ---------------------------------------------------------
    # Register forward hooks for selected modules
    # ---------------------------------------------------------
    name_to_module = dict(hw_model.named_modules())
    ...
    for mod_name in modules_to_test:
        if mod_name not in name_to_module:
            raise KeyError(
                f"Module '{mod_name}' not found in hw_model.named_modules()."
            )
    ...
    for mod_name in modules_to_test:
        ...
        y_hw = captured.get(mod_name, None)
        if y_hw is None:
            print(f"[WARN] No HW activation captured for '{mod_name}'; skipping.")
            continue
        ...
        y_hw_cpu = y_hw.cpu()
        res = compare_tensors(mod_name, y_gold, y_hw_cpu)
        results.append(res)

    # ---------------------------------------------------------
    # Summary
    # ---------------------------------------------------------
    print("========== TinyViT HW vs GOLDEN SUMMARY ==========")
    if not results:
